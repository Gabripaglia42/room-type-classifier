...
# Third-party code: see THIRD_PARTY_NOTICES.md (not covered by the Apache License).
"""Network definition of the CubiCasa5K multi-task model (hg_furukawa_original).

Vendored from https://github.com/CubiCasa/CubiCasa5k (floortrans/models/
hg_furukawa_original.py), unchanged except that the init_weights() method and
its import of the human-pose pretraining weights were removed: inference loads
the complete CubiCasa5K checkpoint instead.

CubiCasa5K is licensed CC BY-NC 4.0 (non-commercial use only).
Kalervo et al., "CubiCasa5K: A Dataset and an Improved Multi-Task Model for
Floorplan Image Analysis", SCIA 2019.
"""
import torch
